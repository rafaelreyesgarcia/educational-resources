...
- proxy patterns can be implemented but is hard
...
compilation is deterministic, always produces the same output
...
5. if the metadata hashes at the end of the bytecode match contract gets fully verified
...
**storage contract**
holds the state associated with the contract user balances and addresses
...
users must trust developers to not modify contracts maliciously, defeats the purpose of trustlesness
...
RPC interface has eth_estimateGas method that will give an estimate

the data in params is the bytecode from the address deploying
...
blockchain-based persistence is achieved by runninodes of a network 

contract-based persistance relies on the intuitithat data can't be replicated by every node and storforever, so instead it is upkept with contraagreements
