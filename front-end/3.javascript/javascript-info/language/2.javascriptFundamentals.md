HELLO WORLD
  ...
  script tags 
    ...
    external scripts
      src attribute with the path to the js file as value
      browsers download external files and store them in cache
      other pages can reference the same script from the cache instead of downloading it every time
        
      referencing an external file,
      disables reading the content of the script tag
...
CODE STRUCTURE
  ...
  statements
    ...
    can be omitted when a line break exists
      (automatic semicolon insertion)
    in most cases it will be inserted not always
...
THE MODERN MODE "USE STRICT"    
  ...
  "use strict"
    ...
    can also be used in just local scope
    required to be placed at the top of the scope
...
VARIABLES
  ...
  var is the older keyword to declare variables in js
...
DATA (VALUE) TYPES
  ...
  Primitive types
    ...
    Number
      ...
      special numeric values
        ...
        NaN
          ...
          propagates, any operation done with NaN     will return NaN

    BigInt

      the number type can't represent integer values  larger than 2^53-1 or -(2^53-1)
      ...
      a BigInt value is creating by appending n to the    end of an integer
    ...
    undefined 
      ...
      an be explicitly assigned to a variable
  ...
  operators
    ...
    arithmetic operators
      ...
      exponentiation **
        ...
        also applies to non-integer numbers (fractions)
        4 ** (1/2)
          same as square root
        8 ** (1/3)
          same as cubic root
